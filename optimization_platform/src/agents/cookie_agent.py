...
class CookieAgent(object):
    ...
    @classmethod
    def register_cookie_for_client(cls, data_store, client_id, session_id, shopify_s, cart_token, creation_time):
        table = TABLE_COOKIES

        creation_time_utc_str = DateUtils.convert_timestamp_to_utc_iso_string(creation_time)
        columns_value_dict = {"client_id": client_id,
                              "session_id": session_id, "shopify_s": shopify_s, "cart_token": cart_token,
